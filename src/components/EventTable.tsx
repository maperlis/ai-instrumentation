...
import { Pencil, Trash2, Save, X, Target } from "lucide-react";
...
export const EventTable = ({ 
  ...
  // Extract related metrics from event (from implementation_notes or dedicated field)
  const getRelatedMetrics = (event: TaxonomyEvent): string[] => {
    // Check if event has explicit related_metrics field
    if (event.related_metrics && Array.isArray(event.related_metrics)) {
      return event.related_metrics;
    }
    
    // Try to match event to selected metrics based on naming patterns
    const eventName = event.event_name?.toLowerCase() || '';
    const description = event.description?.toLowerCase() || '';
    const notes = event.implementation_notes?.toLowerCase() || '';
    
    const matchedMetrics = selectedMetrics.filter(metric => {
      const metricLower = metric.toLowerCase();
      const metricWords = metricLower.split(/[\s_-]+/);
      
      // Check if metric name appears in event details
      return metricWords.some(word => 
        word.length > 3 && (
        ...
        <Table>
          ...
          <TableBody>
            {events.map((event, index) => {
              const relatedMetrics = getRelatedMetrics(event);
                  ...
                  {editingIndex === index ? (
                    <>
                      ...
                      {selectedMetrics.length > 0 && (
                        <TableCell>
                          <Input
                            value={editedEvent?.related_metrics?.join(", ") || relatedMetrics.join(", ")}
                            onChange={(e) =>
                              setEditedEvent({
                                ...editedEvent!,
                            ...
                            placeholder="Enter related metrics..."
                            className="min-w-[150px]"
                          />
                        </TableCell>
                      )}
                      ...
                      {selectedMetrics.length > 0 && (
                        <TableCell>
                          {relatedMetrics.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {relatedMetrics.slice(0, 2).map((metric, i) => (
                                <Tooltip key={i}>
                                  <TooltipTrigger>
                                    <Badge 
                                      variant="outline" 
                                    ...
                                    >
                                      {metric.length > 20 ? metric.slice(0, 18) + '...' : metric}
                                    </Badge>
                                  </TooltipTrigger>
                                  <TooltipContent>
                                    <p>{metric}</p>
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <div className="space-y-1">
                                      {relatedMetrics.slice(2).map((m, i) => (
                                        <p key={i}>{m}</p>
                                      ))}
                                    </div>
                                  </TooltipContent>
